 ...
 * unexpected error. If middleware throws, requests pass through unchanged
 * rather than 500-ing the site. robots.txt + X-Robots-Tag headers still
 * keep crawlers out in the fail-open case.
 */
...
export function middleware(request: NextRequest) {
  try {
    ...
    // Fail open: never let a middleware bug 500 the whole site.
    console.error("[middleware] caught unexpected error, failing open:", err);
    return NextResponse.next();
  }
}

// Matcher: run on everything except Next.js internals and favicon/robots.
export const config = {
